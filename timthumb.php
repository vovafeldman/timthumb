...
class timthumb {
    ...
    */
    ...
    protected function handleErrors(){
        if($this->haveErrors()){ 
            if((NOT_FOUND_IMAGE || NOT_FOUND_IMAGES_FOLDER) && $this->is404()){
                if($this->serveRandomImg(array('generic'), NOT_FOUND_IMAGES_FOLDER, NOT_FOUND_IMAGE)){
                    exit(0);
                } else {
                    $this->error("Additionally, the 404 image that is configured could not be found or there was an error serving it.");
                }
            }
            if(ERROR_IMAGE || ERROR_IMAGES_FOLDER){
                if($this->serveRandomImg(array('generic'), ERROR_IMAGES_FOLDER, ERROR_IMAGE)){
                    exit(0);
                } else {
    ...
    protected function getLocalImagePath($src){
        $src = ltrim(ltrim($src, '/'), '\\'); //strip off the leading '/'
        if(! $this->docRoot){
...
*/
    ...
    protected function serveRandomImg($tags = array(), $dir = RANDOM_IMAGES_FOLDER, $file = DEFAULT_IMAGE)
    {
        $this->src = DEFAULT_IMAGE;
        if ($handle = opendir(RANDOM_IMAGES_FOLDER))
        {
            $files = glob($dir . '/*.jpg');
            ...
            $this->src = $files[$file];
            if (0 === strpos($this->src, $this->docRoot))
                $this->src = ltrim(substr($this->src, strlen($this->docRoot)), '/');
            $this->localImage = $this->getLocalImagePath($this->src);
            $this->isURL = false;
            /*if(! $this->localImage){
                $this->debug(1, "Could not find the local image: {$this->localImage}");
